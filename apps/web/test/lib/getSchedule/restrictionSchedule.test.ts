  ...
  describe("Restriction Schedule", () => {
    test("should respect date override rule in restrictionSchedule (Europe/London, useBookerTimezone=false)", async () => {
      ...
      const expectedUtcSlots = [
        ...
        // REVIEW: Is this slot expected to be available?
        "2025-06-02T15:00:00.000Z", // 4 PM London, 7 PM Dubai, 8:30 PM Kolkata
      ];
      ...
      expect(result.slots["2025-06-03"]?.length).toBeGreaterThan(0);
    });

    // REVIEW: Handles acc. to new recurring event logic.
    ...
    test("should respect recurring rule in restrictionSchedule (Europe/London, useBookerTimezone=true)", async () => {
      ...
      const getExpectedSlotsForDate = (dateString: string) => [
        ...
        // REVIEW: Is this slot expected to be available.
        `${dateString}T13:00:00.000Z`, // 6:30 PM Kolkata, 2 PM London
        `${dateString}T14:00:00.000Z`, // 7:30 PM Kolkata, 3 PM London
        `${dateString}T15:00:00.000Z`, // 8:30 PM Kolkata, 4 PM London
      ];
